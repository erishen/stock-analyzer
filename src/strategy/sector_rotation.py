...
DEFAULT_SECTORS = {
    ...
    "证券": ["sh600030", "sh600837", "sh600999", "sh601211", "sh601688", "sh601901"],
    "保险": ["sh601318", "sh601601", "sh601628"],
    "房地产": ["sz000002", "sh600048", "sh600383", "sz001979"],
    "建筑": ["sh600068", "sh601186", "sh601390", "sh601668"],
    "钢铁": ["sh600019", "sh600782", "sh601003"],
    "煤炭": ["sh601088", "sh601225", "sh601666"],
    "石油": ["sh600028", "sh601857"],
    "电力": ["sh600011", "sh600886", "sh600900", "sh601985"],
    "汽车": ["sh600104", "sh600066", "sh601238", "sh601633", "sz002594"],
    "家电": ["sz000333", "sz000651", "sh600690"],
    "食品饮料": ["sz000568", "sz000858", "sh600519", "sh600887", "sh603288"],
    "医药": ["sz000538", "sh600276", "sh600196", "sz000661", "sz300760"],
    "白酒": ["sz000568", "sz000596", "sz000858", "sh600519", "sh600809"],
    "半导体": ["sz002049", "sz002371", "sh603501", "sh603986", "sh688981"],
    "计算机": ["sz002230", "sz002410", "sz002415", "sz300033", "sz300059", "sh688111"],
    "通信": ["sz000063", "sz002475", "sh600050", "sh603160"],
    "电子": ["sz000725", "sz002241", "sz002415", "sz002475", "sh603160"],
    "传媒": ["sz000156", "sz002602", "sz300059", "sh601801"],
...
class SectorRotationAnalyzer:
    ...
    def calculate_sector_metrics(
        ...
        # 兼容带前缀(sh/sz)与裸代码两种库内格式
        bare = [s[2:] if s[:2] in ("sh", "sz") else s for s in stocks]
        match_codes = list(dict.fromkeys(stocks + bare))
        placeholders = ",".join(["?" for _ in match_codes])
        query = f"""
            SELECT code, change_percent, volume, close, ma5, ma20
            FROM stock_analysis
            WHERE date = ? AND code IN ({placeholders})
        """
        params = [date, *match_codes]
